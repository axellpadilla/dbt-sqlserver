...
_microbatch_model_alias_join_sql_datetime = """
{{ config(materialized='incremental', incremental_strategy='microbatch',
unique_key='id', event_time='event_time', batch_size='day', begin='2020-01-01 00:00:00') }}
select A.id, A.event_time
from {{ ref('input_model') }} as A
inner join {{ ref('input_model') }} as B
    on A.id = B.id
"""
...
class TestSQLServerMicrobatchDateTimeAliasJoin(BaseMicrobatch):
    @pytest.fixture(scope="class")
    def microbatch_model_sql(self) -> str:
        return _microbatch_model_alias_join_sql_datetime
