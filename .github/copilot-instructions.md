...
Repository layout and branch context
- This repository comes from a fork of upstream dbt-sqlserver.
- Keep this file aligned with the current checked-in layout and update it when paths, commands, or workflows change.
...
- Key responsibilities:
  ...
  - SQL helper macros & materializations: `dbt/include/sqlserver/macros/…`, including `dbt/include/sqlserver/macros/materializations/…` (use these as canonical SQL patterns)
...
- Local verification: `make test` runs unit tests plus the staged black, flake8, and mypy checks.
...
- To expose or change macros/materializations: update files under `dbt/include/sqlserver/macros/**`.
...
- If the change impacts connection images or CI, update `devops/CI.Dockerfile`, `devops/server.Dockerfile`, `docker-compose.yml`, and `.github/workflows/*` accordingly.
